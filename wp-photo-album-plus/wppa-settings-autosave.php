				...
				if ( is_array( $file ) ) {
					if ( $file['error'] ) {
						...
						if ( !is_array($imgsize) || !isset($imgsize[2]) || $imgsize[2] != 3 ) {
							...
							/* translators: filename */
							wppa_alert_text(sprintf(__('Upload of %s done', 'wp-photo-album-plus' ), strtolower(sanitize_file_name(basename($file['name'])))));
						}
					}
				}
				else {
				...
				if ( is_array( $file ) ) {
					...
					if ( $file['error'] ) {
						...
						if ( substr(sanitize_file_name($file['name']), -4) != '.ttf' ) {
							...
							/* translators: filename */
							wppa_alert_text(sprintf(__('Upload of %s done', 'wp-photo-album-plus' ), sanitize_file_name(basename($file['name']))));
						}
					}
				}
				else {
...
//				if ( isset( $_FILES['file_1'] ) && isset( $_FILES['file_1']['error'] ) && $_FILES['file_1']['error'] != 4 ) {
//					$file = wp_unslash( $_FILES['file_1'] );
				$file = current( $_FILES ); // ['file_1'];
				if ( is_array( $file ) ) {
					if ( $file['error'] ) {
						/* translators: errorcode */
...
//					$file = $_FILES['file_1'];
	...
	<div class="wrap">
		...
		if ( ! wppa_opt( 'image_magick' ) ) {
			if ( class_exists( 'Imagick' ) && function_exists( 'exec' ) ) {
				...
				if ( ! empty( $result ) ) {
					...
					/* translators: file system location */
				...
				wppa_update_option( 'wppa_image_magick', 'none' );
			}
		}
		...
		if ( $tab == 'share' || $tab == 'links' ) {
			...
			if ( $pages ) {
				...
				foreach ( array_keys($pages) as $index ) {
					$pages[$index]['post_title'] = wppa_translate( stripslashes( $pages[$index]['post_title'] ), 'wp-photo-album-plus' );
				}

				$pages = wppa_array_sort($pages, 'post_title');
				foreach ($pages as $page) {
					if (strpos($page['post_content'], '%%wppa%%') !== false || strpos($page['post_content'], '[wppa') !== false) {
						$opts_page[] = wppa_translate($page['post_title'], 'wp-photo-album-plus' );
						$opts_page_post[] = wppa_translate($page['post_title'], 'wp-photo-album-plus' );
						$vals_page[] = $page['ID'];
		...
		wppa_echo( '<ul class="widefat wppa-setting-tabs">' );
			...
			wppa_setting_tab( 'share', 			$wppa_tab_names['share'], 		$advan );
			...
			switch($tab) {
				...
				case 'layout': {
					// General layout settings
					{
						...
						$name = __('Screen and viewport sizes', 'wp-photo-album-plus');
						$desc = '';
						$help = '';
						$slug = 'wppa_dummy';
						$html = '<table>
									<tr>
										<td>Screen width:</td><td id="wppa-sw"></td><td>Screen height:</td><td id="wppa-sh"></td>
									</tr>
									<tr>
										<td>Window width:</td><td id="wppa-ww"></td><td>Window height:</td><td id="wppa-wh"></td>
									</tr>
									<tr>
										<td>wppaScreenWidth:</td><td id="wppa-wsw"></td><td>wppaScreenHeight:</td><td id="wppa-wsh"></td>
									</tr>
								</table>';
						wppa_setting_new($slug, '13', $name, $desc, $html, $help);
					...
					// Navigation symbol specifications
					{
						...
						$name = __('Navigation symbols size mobile', 'wp-photo-album-plus');
						$desc = __('Navigation button size on mobile devices', 'wp-photo-album-plus');
						$help = '';
						$slug = 'wppa_nav_icon_size_mobile';
						$opts = array(	'16px',
										'20px',
										'24px',
										'32px',
				...
				case 'photos': {
					// Photo specifications
					{
						...
						$help = '';
						$slug = 'wppa_zoom_on';
						$onch = "wppaSlaveChecked(this,'wppa_zoom_no_widget');";
						$html = wppa_checkbox($slug, $onch);
						wppa_setting_new($slug, '4', $name, $desc, $html, $help);

						$name = __('No zoom in widgets', 'wp-photo-album-plus');
						$desc = __('Prevent zooming photos in widgets', 'wp-photo-album-plus');
						$help = '';
						$slug = 'wppa_zoom_no_widget';
						$html = wppa_checkbox($slug);
						wppa_setting_new($slug, '5', $name, $desc, $html, $help, wppa_switch('zoom_on'));
					...
					// Photo of the day settings
					{
						...
						$html = '
								<div style="float:left">' .
									...
									sprintf( __('Current day of the week = %1$s, day of the month = %2$s, day of the year = %3$s, offset = ', 'wp-photo-album-plus' ),
...
//					}
				...
				case 'slide': {
					// Sllideshow component specifications
					{
						...
						$slug = 'wppa_show_ubb';
						$html = wppa_checkbox($slug) . '<span style="float:left">&nbsp;' . __('New style', 'wp-photo-album-plus') . ':&nbsp;</span>' . wppa_checkbox('wppa_ubb_new');
						...
						$html = wppa_checkbox($slug) . '<span style="float:left">&nbsp;' . __('New style', 'wp-photo-album-plus') . ':&nbsp;</span>' . wppa_checkbox('wppa_startstop_new');
...
*/
				...
				case 'comments': {
					// Comments system related settings
					{
						...
						$name = __('Admin edit link', 'wp-photo-album-plus');
						$desc = __('Add edit comment link for admin users', 'wp-photo-album-plus');
						$help = '';
						$slug = 'wppa_edit_com_link';
						$html = wppa_checkbox($slug);
						wppa_setting_new($slug, '18', $name, $desc, $html, $help);
				...
				case 'search': {
					// Search albums and photos features related settings
					{
						...
						if ($pages) {
							...
							// Translate
							foreach ( array_keys($pages) as $index ) {
								$pages[$index]['post_title'] = wppa_translate( stripslashes( $pages[$index]['post_title'] ) );
							}
							$pages = wppa_array_sort($pages, 'post_title');
							foreach ($pages as $page) {
								if ( strpos($page['post_content'], '%%wppa%%') !== false || strpos($page['post_content'], '[wppa') !== false ) {
									$opts[] = wppa_translate( $page['post_title'] );
									$vals[] = $page['ID'];
								}
								else {
									$opts[] = '|' . wppa_translate( $page['post_title'] ) . '|';
						...
						/* translators: integer */
						$name = sprintf(__('Box %d caption', 'wp-photo-album-plus' ), '2');
						$desc = __('Enter caption text', 'wp-photo-album-plus' );
						$help = '';
						...
						/* translators: integer */
						$name = sprintf(__('Box %d content', 'wp-photo-album-plus' ), '2');
						$desc = __('Enter search tokens, one per line.', 'wp-photo-album-plus' );
						$help = '';
						...
						/* translators: integer */
						$name = sprintf(__('Box %d caption', 'wp-photo-album-plus' ), '3');
						$desc = __('Enter caption text', 'wp-photo-album-plus' );
						$help = '';
						...
						/* translators: integer */
						$name = sprintf(__('Box %d content', 'wp-photo-album-plus' ), '3');
						$desc = __('Enter search tokens, one per line.', 'wp-photo-album-plus' );
						$help = '';
				...
				case 'links': {
					...
					// Other links
					{
						...
						$coldef = array( 	__('#', 'wp-photo-album-plus' ) => '24px;',
											...
											__('Description', 'wp-photo-album-plus' ) => 'auto;',
											__('Link type', 'wp-photo-album-plus' ) => 'auto;',
											__('Display type', 'wp-photo-album-plus' ) => 'auto;',
											__('Landing page', 'wp-photo-album-plus' ) => 'auto;',
						...
						$html = wppa_checkbox($slug);
						wppa_setting_new($slug, '7', $name, $desc, $html.'</td><td></td><td></td><td></td><td>', $help);
						...
						$html = wppa_checkbox($slug);
						wppa_setting_new($slug, '8', $name, $desc, $html.'</td><td></td><td></td><td></td><td>', $help);
						...
						$slug1 = '';
						$slug2 = '';
						$slug3 = 'wppa_super_view_linkpage';
						wppa_verify_page($slug3);
						$slug4 = '';
						$slug5 = '';
						$slug = array($slug1, $slug2, $slug3, $slug4, $slug5);
						$html1 = __('Defined by the visitor: slideshow or thumbnails', 'wp-photo-album-plus' );
						$html2 = '';
						$html3 = wppa_select($slug2, $opts_page_auto, $vals_page);
						$html4 = '';
						$html5 = '';
						$html = array($html1, $html2, $html3, $html4, $html5);
						wppa_setting_new($slug, '9', $name, $desc, $html, $help);
						...
						$slug1 = '';
						$slug2 = '';
						$slug3 = 'wppa_upldr_widget_linkpage';
						wppa_verify_page($slug3);
						$slug4 = '';
						$slug5 = '';
						$slug = array($slug1, $slug2, $slug3, $slug4, $slug5);
						$html1 = '';
						$html2 = '';
						$html3 = wppa_select($slug3, $opts_page_auto, $vals_page);
						$html4 = '';
						$html5 = '';
						$html = array($html1, $html2, $html3, $html4, $html5);
						wppa_setting_new($slug, 10, $name, $desc, $html, $help);
						...
						$slug1 = '';
						$slug2 = '';
						$slug3 = 'wppa_supersearch_linkpage';
						wppa_verify_page($slug3);
						$slug4 = '';
						$slug5 = '';
						$slug = array($slug1, $slug2, $slug3, $slug4, $slug5);
						...
						if ( count( $mails ) ) {
							...
							foreach( $mails as $mail ) {
								...
								<tr class="wppa-setting wppa-IX-X" >
									...
									<td>' . $mail['to'] . '</td>
									<td>' . $mail['subj'] . '</td>
									<td style="max-width:40%" >' . wp_strip_all_tags( $mail['message'] ) . '</td>
									<td>' . $mail['retry'] . '</td>
								</tr>' );
							}
				...
				case 'share': {
					...
					// Search Engine Optimalisation settings
					{
						...
						$name = __('Void meta keywords', 'wp-photo-album-plus');
						$desc = __('Exclude these words from meta keywords', 'wp-photo-album-plus');
						$help = __('Default voided keywords are', 'wp-photo-album-plus').': '.wppa_get_keywords(0,true);
						$slug = 'wppa_meta_void_custom';
				...
				case 'system': {
					// System behaviour related settings
					{
						...
						$name = __('Frontend Ajax method', 'wp-photo-album-plus' );
						$desc = __('The method Ajax will use at the frontend', 'wp-photo-album-plus' );
						...
						$opts = array( 	__('Using REST API', 'wp-photo-album-plus' ),
										__('By redirection', 'wp-photo-album-plus' ),
										__('Backend method', 'wp-photo-album-plus' ),
										__('None', 'wp-photo-album-plus' ),
										);
						...
						wppa_setting_new($slug1, '5', $name, $desc, $html1.$html2, $help);
						...
						$help .= '<br>' .
									/* translators: link */
									sprintf( __( 'Click %1$s here %2$s for documentation and a full list of available options', 'wp-photo-album-plus' ),
...
*/
				...
				case 'new': {
					// New albums / photos related settings
					{
						...
						/*
						$name = __('LasTen item sequence random', 'wp-photo-album-plus');
						$desc = __('Use random order rather than descending timestamp', 'wp-photo-album-plus');
						$help = '';
						$slug = 'wppa_lasten_random';
						$html = wppa_checkbox($slug);
						wppa_setting_new($slug, '11a', $name, $desc, $html, $help);
						*/
				...
				case 'admin': {
					...
					// Frontend create albums and upload Photos enabling and limiting settings
					{
						...
						$help = '';
						$slug1 = 'wppa_user_album_edit_on';
						$slug2 = 'wppa_user_album_parent_on';
						$html1 = wppa_checkbox($slug1);
						$html2 = wppa_checkbox($slug2) . ' ' . __( 'Select parent', 'wp-photo-album-plus' );
						$html = array( $html1, $html2 );
						wppa_setting_new($slug, '5', $name, $desc, $html, $help);
					...
					// Miscellaneous limiting settings
					{
						...
						$slug0 = 'wppa_fe_edit_move';
						$slug1 = 'wppa_fe_edit_name';
						$slug2 = 'wppa_fe_edit_desc';
						$slug3 = 'wppa_fe_edit_tags';
						$html0 = ' <span style="float:left" >'.__('Album', 'wp-photo-album-plus' ).':</span>'.wppa_checkbox($slug0);
						...
						$html = array($html0.$html1.$html2.$html3);
				...
				case 'maintenance': {
					// Regular maintenance procedures
					{
						...
						$files0 = wppa_glob(WPPA_PATH.'/theme/*.skin'); 	// factory supplied
						$files1 = wppa_glob(WPPA_UPLOAD_PATH.'/backup/*.bak');
						$files2 = wppa_glob(WPPA_UPLOAD_PATH.'/backup/*.skin');
						$files3 = wppa_glob(WPPA_DEPOT_PATH.'/*.bak'); 		// for backward compat
						$files4 = wppa_glob(WPPA_DEPOT_PATH.'/*.skin'); 	// for backward compat
						$files = array_merge( $files0, $files1, $files2, $files3, $files4 );
						$opts = array();
						$vals = array();
						...
						$name = __('Lost and found 1', 'wp-photo-album-plus' );
						$desc = __('Find "lost" photos.', 'wp-photo-album-plus' );
						$help = __('This function will attempt to find lost photos that have no entry in the db', 'wp-photo-album-plus' );
						$slug2 = 'wppa_cleanup_a';
						$html1 = '';
						$html2 = wppa_maintenance_button( $slug2 );
						$html3 = wppa_status_field( $slug2 );
						$html4 = wppa_togo_field( $slug2 );
						$html = array($html1, $html2, $html3, $html4);
						wppa_setting_new(false, '6a', $name, $desc, $html, $help);

						$name = __('Lost and found 1', 'wp-photo-album-plus' );
						$desc = __('Find "lost" photos.', 'wp-photo-album-plus' );
						$help = __('This function will attempt to find lost photos that have a non existent album in the db', 'wp-photo-album-plus' );
						$slug2 = 'wppa_cleanup_b';
						$html1 = '';
						$html2 = wppa_maintenance_button( $slug2 );
						$html3 = wppa_status_field( $slug2 );
						$html4 = wppa_togo_field( $slug2 );
						$html = array($html1, $html2, $html3, $html4);
						wppa_setting_new(false, '6b', $name, $desc, $html, $help);
					...
					// One time conversions
					{
						...
						$name = __('Convert db field specs', 'wp-photo-album-plus');
						$desc = __('Convert datatypes in the wppa db tables to take less space', 'wp-photo-album-plus');
						$help = __('Run this when you have persistent database problems', 'wp-photo-album-plus');
						$slug2 = 'wppa_fix_datatypes';
						$html1 = wppa_cronjob_button( $slug2 );
						$html2 = wppa_maintenance_button( $slug2 );
						$html3 = wppa_status_field( $slug2 );
						$html4 = wppa_togo_field( $slug2 );
				...
				case 'watermark': {
					// Watermark related settings
					{
						...
						/* translators: integer percentage */
	...
	<div style="clear:both;margin-top:12px">
		<br>' .
		/* translators: float memory size in mb */
