		...
		case 'wppa_album_photos':
			{
				...
				function wppaConvertToAlbum(id,paging,contin,stop) {
					var ajaxurl = wppaAdminAjaxUrl + "?action=wppa&wppa-action=pdftoalbum&photo-id=" + id + "&wppa-nonce=" + jQuery( "#photo-nonce-" + id ).val();
		...
		case 'wppa_album_photos_sequence':
			{
				...
				function wppaDoSeqUpdate( photo, seqno ) {
					...
					xmlhttp.open("POST",wppaAdminAjaxUrl,true);
		...
		case '_wppa_admin':
			{
				...
				function wppaGetCoverPreview( albumId, divId ) {
					...
					jQuery.ajax( {
						...
						url: 		wppaAdminAjaxUrl,
						data: 		"action=wppa" +
									"&wppa-action=getshortcodedrendered" +
									"&wppa-from=cover" +
									"&shortcode=[wppa type=\"cover\" album=\""+alb+"\"]",
						async: 		true,
						type: 		"GET",
						timeout: 	10000,
						beforeSend: function( xhr ) {
		...
		case 'wppa_ajax_import_upload':
			{
				...
				function wppaGetUploadOptions() {
					...
					var options = {
						...
						complete: function(response) {
							...
							if ( success == "0" ) {
								jQuery("#message").html( response.responseText.substr(1) );
								jQuery("#bar").css("backgroundColor","#F77");
								jQuery("#percent").html(\''.__('Upload failed', 'wp-photo-album-plus').'\');
							}
				...
				function wppaDisplaySelectedFilesForImport(id) {
					...
					if ( theFiles[0].files.length > maxfiles ) {
						...
						sprintf( __( 'Too many files selected, only %d allowed', 'wp-photo-album-plus' ), ini_get( 'max_file_uploads' ) ) . '";
						ok = false;
					}
		...
		case '_wppa_page_import':
			{
				...
				function wppaDoAjaxImport(from) {
					...
					var data = "wppa-import-submit=ajax&import-ajax-file="+elm.title+"&type="+type;
					var tmp;
					switch ( type ) {
					...
					data += "&wppa-nonce="+jQuery(\'#wppa-import-nonce\').val();
					...
					jQuery.ajax( {
						...
						url: 		wppaAdminAjaxUrl+"?action=wppa&wppa-action=import",
						data: 		data,
						async: 		true,
						type: 		"POST",
						timeout: 	(3600*1000),
						...
						success: 	function( resp, status, xhr ) {
										...
										try { 	// Looks valid json reply
											...
											if ( code > 0 ) {
												...
												if ( code == 1 || code == 9 || code == 14 || code == 15 || code == 33 || code == 36 ) {
				...
				if ( wppa_switch( 'import_auto' ) ) {
					$url = set_url_scheme( 'http://' . wppa_http_host() . wppa_request_uri() );
