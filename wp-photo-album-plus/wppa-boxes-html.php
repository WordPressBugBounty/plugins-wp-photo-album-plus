...
global $wpdb;
	...
	if ( count( $albarr ) == 1 ) {
		$result = '
		<h3>' .
			/* translators: contest name = album name */
						...
						<td class="wppa-contest-table-photo" >
							...
							>';
								...
								// Photo
									...
									$result .=
									wppa_html_tag( 'img', ['src' => wppa_get_photo_url( $id ), 'alt' => $anon ? '' : wppa_get_imgalt( $id, false, true )] );
								}
...
global $photos_used;
	...
	if ( ! empty( $iptclist ) ) {
		foreach( array_keys( $iptclist ) as $idx ) {
			$iptclist[$idx]['description'] = wppa_translate( $iptclist[$idx]['description'] );
		}
	}
	...
	if ( ! empty( $exiflist ) ) {
		foreach( array_keys( $exiflist ) as $idx ) {
			$exiflist[$idx]['description'] = wppa_translate( $exiflist[$idx]['description'] );
		}
	}
	...
	<form
		...
		<select
			...
			foreach ( $exiflist as $item ) {
				...
				$desc = wppa_translate( $item['description'], 'wp-photo-album-plus' );
				if ( substr( $desc, 0, 12 ) != 'UndefinedTag' ) {
					$options_array[] = array( 'tag' => $tag, 'desc' => trim( wppa_translate( $item['description'], 'wp-photo-album-plus' ), ': ' ) );
				}
			}
		...
		// The spinner
		wppa_echo( wppa_html_tag( 'img', ['id' => 'wppa-ss-spinner-'.$mocc, 'src' => wppa_get_imgdir('spinner.gif'), 'style' => "margin:0 4px;display:none;"] ) );
		...
		foreach ( $tags as $tag ) {
			if ( ! $seltags || in_array( $tag['tag'], $selarr ) ) {
				...
				<a
					...
					>' .
					ucfirst( wppa_translate( $name ) ) . '
				</a> ';
			}
		}
		if ( wppa_switch( 'empty_tags' ) ) {
			$href = $hr . '&amp;wppa-tag=-none-';
			$result .=
			wppa_html_tag( 'a', ['href' => $href, 'title' => __('Items without tags', 'wp-photo-album-plus'), 'style' => "font-size:1em;"], wppa_translate( '-none-' ) );
		}
	}
...
function wppa_get_multitag_html( $nperline = '2', $seltags = '' ) {
	...
	if ( wppa_switch( 'empty_tags' ) ) {
		$tags['-none-'] = array ( 'tag' => '-none-', 'ids' => array (), 'count' => 0, 'fraction' => '0', );
	}
			...
			if ( ! $seltags || in_array( $tag['tag'], $selarr ) ) {
				...
				<td
					...
					<input
						...
						$checked . '
					/>&nbsp;' .
					str_replace( ' ', '&nbsp;', ucfirst( wppa_translate( $tag['tag'] ) ) ) . '
				</td>';
				$count++;
				if ( $count % $nperline == 0 ) {
					$result .= '</tr>';
...
global $wppa_fb_init_done;
	...
	$photo_desc = strip_shortcodes( wp_strip_all_tags( $photo_desc ) );
		...
		<div
			...
			<a ' .
				...
				if ( wppa_switch( 'twitter_black' ) ) {
					...
					$stl = "height:' . $s . 'px;vertical-align:top;border-radius:50%;background-color:black";
				}
				else {
					$src = wppa_get_imgdir() . 'twitter.png';
					$stl = "height:' . $s . 'px;vertical-align:top;";
				}
				$tw .= wppa_html_tag( 'img', ['src' => $src, 'style' => $stl, 'alt' => __( 'Share on Twitter', 'wp-photo-album-plus' )] );
				$tw .= '
			</a>
		</div>';
	}
		...
		<div class="wppa-share-icon" style="float:left; padding:0 2px">
			<a ' .
				...
				if ( wppa_switch( 'pinterest_black' ) ) {
					...
					$stl = "height:" . $s . "px;vertical-align:top;border-radius:" . $br . ";";
				}
				$pi .= wppa_html_tag( 'img', ['src' => $src, 'style' => $stl, 'alt' => __( 'Share on Pinterest', 'wp-photo-album-plus' )] );
				$pi .= '
			</a>
		</div>';
	}
	...
	if ( wppa_switch( 'share_linkedin' ) && $key != 'thumb' && $key != 'lightbox' ) {
		...
		<div class="wppa-share-icon" style="float:left; padding:0 2px">
			<a ' .
				...
				$li .= wppa_html_tag( 'img', ['src' => $src, 'style' => $stl, 'alt' =>  __( 'Share on LinkedIn' ,'wp-photo-album-plus' )] );
				$li .= '
			</a>
		</div>';
	}
	...
	if ( wppa_switch( 'share_twitter' ) ) {
		...
		<div class="wppa-share-icon" style="float:left; padding:0 2px">
			<a ' .
				...
				target="_blank"
				>' .
				wppa_html_tag( 'img', ['src' =>  wppa_get_imgdir() . 'twitter.png', 'style' => "height:' . $s . 'px;vertical-align:top;", 'alt' => __( 'Share on Twitter', 'wp-photo-album-plus' )] ) . '
			</a>
		</div>';
	}
	else {
		...
		<div
			...
			<a
				...
				onclick="wppaStopProp(event);
					...
					wppaAjaxDestroyAlbum(\'' . wppa_encrypt_album( $alb ) . '\',\'' . wp_create_nonce( 'wppa-nonce_' . $alb ) . '\');
...
function wppa_get_user_create_html( $alb, $width, $where = '', $mcr = false ) {
	...
	$result .= '
		...
		<a id="_wppa-cr-' . $a . '-' . $mocc . '" class="wppa-create-' . $where . ' wppa-album-cover-link" onclick="
			...
			wppa_translate( wppa_opt( 'close_text' ), 'wp-photo-album-plus' ) . '
		</a>';
			...
			<form
				...
				if ( ( $where == 'widget' || wppa( 'is_upload' ) ) && ! wppa_switch( 'default_parent_always' ) ) {
					...
					<select
						...
						style="max-width: 100%;"
						>' .
						wppa_album_select_a( array ( 	'void' 				=> '',
														'selected' 			=> '',
														'disabled' 			=> '',
...
static $albums_granted;
	...
	$result .= '
		...
		<div id="wppa-file-' . $t . $a . '-' . $mocc . '" style="width:100%;text-align:center;display:none; clear:both" onclick="wppaStopProp(event);">
			<form id="wppa-uplform-' . $cralb . '-' . $mocc . '" action="' . $returnurl . '" method="post" enctype="multipart/form-data" >' .
				wppa_nonce_field( 'wppa-check', 'wppa-nonce', false, $yalb );

	// Single Album given
	if ( wppa_is_int( $alb ) ) {
		$result .= '
			<input type="hidden" id="wppa-upload-album-' . $mocc . '-' . $seqno . '" name="wppa-upload-album" value="' . $cralb . '" />';
		...
		// No, there are too many albums
			...
			$body .= '
				<input
					...
					name="wppa-upload-album"
					style="max-width: 100%;"
					onchange="jQuery( \'#wppa-sel-'.$cralb.'-'.$mocc.'\' ).trigger( \'onchange\' )"
				/>';
		}
	}
	...
	<input
		...
		( $multiple ? ' multiple' : '' ) . '
		style="display:none;"
		id="wppa-user-upload-' . $cralb . '-' . $mocc . '"
		name="wppa-user-upload-' . $yalb . '-' . $mocc . '[]"
		onchange="
			jQuery( \'#wppa-user-submit-' . $cralb . '-' . $mocc.'\' ).css( \'display\', \'block\' );
			jQuery( window ).trigger(\'resize\');
			wppaDisplaySelectedFiles(\'wppa-user-upload-' . $cralb . '-' . $mocc . '\');
			"
	/>';
	...
	<input
		...
		id="wppa-user-upload-' . $cralb . '-' . $mocc . '-display"
		class="wppa-upload-button"
		value="' . $value . '"
		onclick="
			wppaSetMaxWidthToParentWidth(this);
			jQuery(\'#wppa-user-upload-' . $cralb . '-' . $mocc . '\').removeAttr(\'capture\');
			jQuery(\'#wppa-user-upload-' . $cralb . '-' . $mocc . '\').click();
			"
	/>';
	...
	if ( wppa_is_iphoneoripad() && $on_camera ) {
		...
		<input
			...
			style="
				...
				"
			id="wppa-user-upload-' . $cralb . '-' . $mocc . '-idisplay"
			class="wppa-upload-button"
			value="&nbsp;"
			onclick="
				jQuery(\'#wppa-user-upload-' . $cralb . '-' . $mocc . '\').attr(\'capture\',\'environment\');
				jQuery(\'#wppa-user-upload-' . $cralb . '-' . $mocc . '\').click();
				"
		/>';
	}
	...
	if ( wppa_switch( 'copyright_on' ) ) {
		$body .=
			'<div style="width:100%;clear:both">' .
				wppa_translate( wppa_opt( 'copyright_notice' ) ) .
			'</div>';
	}
	...
	if ( wppa_switch( 'desc_user' ) ) {
		...
		// Do NOT show newphoto des if it contains html for users that may not edit html
		if ( ! current_user_can( 'unfiltered_html' ) ) {
			if ( $desc != wp_strip_all_tags( $desc ) ) {
				$desc = '';
			}
		}
		else {
	...
	if ( wppa_switch( 'fe_custom_fields' ) ) {
		for ( $i = 0; $i < 10 ; $i++ ) {
			if ( wppa_opt( 'custom_caption_' . $i ) ) {
				...
				$head = wppa_translate( wppa_opt( 'custom_caption_' . $i ) ) .
	...
	if ( wppa_switch( 'fe_upload_tags' ) ) {
		...
		$onc = 'wppaPrevTags(' .
				'\'wppa-sel-'.$cralb.'-'.$mocc.'-1\',' .
				'\'wppa-sel-'.$cralb.'-'.$mocc.'-2\',' .
				'\'wppa-sel-'.$cralb.'-'.$mocc.'-3\',' .
				'\'wppa-inp-'.$cralb.'-'.$mocc.'\',' .
				'\'wppa-upload-album-'.$mocc.'-'.$seqno.'\',' .
				'\'wppa-prev-'.$cralb.'-'.$mocc.'\',' .
				'\'wppa-newval-'.$cralb.'-'.$mocc.'\',' .
				'\'wppa-user-submit-'.$cralb.'-'.$mocc.'\',' .
				'\'' . esc_js( __('Wait...', 'wp-photo-album-plus') ) . '\');';

		// Selection boxes 1..3
		for ( $i = 1; $i < '4'; $i++ ) {
			if ( wppa_switch( 'up_tagselbox_on_'.$i ) ) {

				$head = wppa_translate( wppa_opt( 'up_tagselbox_title_'.$i ) );
				$head = trim( $head, ': ');
				$body = '
				<select
					id="wppa-sel-' . $cralb . '-' . $mocc . '-' . $i . '"
		...
		if ( wppa_switch( 'up_tag_input_on' ) ) {
			...
			$head = wppa_translate( wppa_opt( 'up_tag_input_title' ) );
			$head = trim( $head, ': ');
			$body = '
			<input
				id="wppa-inp-' . $cralb . '-' . $mocc . '"
				type="text"
				class="wppa-box-text"
				style="padding:0; width:100%;"
				name="wppa-new-tags"
...
/* start submit section */
	...
	$body =
		...
		'<input' .
			' type="submit"' .
			' id="wppa-user-submit-' . $cralb . '-' . $mocc . '"' .
			$onclick .
			' style="display:none; margin: 6px 0; float:right;"' .
			' class="wppa-user-submit"' .
			' name="wppa-user-submit-'.$cralb.'-'.$mocc.'" value="' . $value . '"' .
		' />' .
		'<div style="height:6px;clear:both;"></div>
		<div' .
			' id="progress-'.$cralb.'-'.$mocc.'"' .
			' class="wppa-progress "' .
			' style="width:100%;border-color:'.wppa_opt( 'bcolor' ).'"' .
			' >' .
			'<div id="bar-'.$cralb.'-'.$mocc.'" class="wppa-bar" ></div>' .
			'<div id="percent-'.$cralb.'-'.$mocc.'" class="wppa-percent">0%</div>' .
		'</div>' .
		'<div id="message-'.$cralb.'-'.$mocc.'" class="wppa-message" ></div>';
...
/* End submit section */
	...
	// Preview area if not visible. Here are the sanitized tags stored get along with the form data
	if ( ! wppa_switch( 'up_tag_preview' ) ) {
		$val  = ( $yalb ? htmlspecialchars( trim( wppa_sanitize_tags( wppa_get_album_item( $yalb, 'default_tags' ), false, true ), ',' ) ) : '' );
		$result .= '<input type="hidden" name="wppa-prev-tags" id="wppa-newval-'.$cralb.'-'.$mocc.'" value="' . $val . '">';
	}
	...
	// Ajax upload script
	wppa_js( 'jQuery(document).ready(function() {jQuery("#wppa-uplform-'.$cralb.'-'.$mocc.'").ajaxForm(wppaGetUploadOptions( "'.$cralb.'", '.$mocc.', "'.$where.'", "'.$on_complete.'" ));});' );
...
//	}
	...
	if ( substr( $name, -3 ) != '[:]' ) {
		$name = wppa_translate( $name );
	}

	// qTranslate(-x) not active or not properly closed tag?
	if ( substr( $desc, -3 ) != '[:]' ) {
		$desc = wppa_translate( $desc );
	}
	...
	$result .=
	'<div id="wppa-fe-div-'.$a.'-'.wppa( 'mocc' ).'" style="display:none" onclick="wppaStopProp(event);">' .
					...
					if ( $comment['status'] == 'approved' ||
						...
						<td
							...
							if ( wppa_opt( 'comment_gravatar' ) != 'none' ) {
								...
								if ( ! $avt ) {
									$avt = wppa_html_tag( 'img', ['class' =>"avatar wppa-box-text wppa-td",
											'src' => 'http'.(is_ssl()?'s':'').'://www.gravatar.com/avatar/'.wppa_get_unique_crypt().'.jpg?d='.urlencode( $default ).'&s='.wppa_opt('gravatar_size'),
						...
						<td
							...
							// Status approved
							if ( ( $comment['status'] != 'approved' || wppa_switch( 'edit_com_link' ) ) && ( current_user_can( 'wppa_moderate' ) || current_user_can( 'wppa_comments' ) ) ) {
								if ( wppa( 'no_esc' ) ) {
									$commentblock .= wppa_moderate_links( 'comment', $id, $comment['id'], $comment['status'] );
								}
								else {
									$commentblock .= wppa_html( esc_js( wppa_moderate_links( 'comment', $id, $comment['id'], $comment['status'] ) ) );
								}
							}
							...
							if ( $comment['status'] == 'pending' ) {
								...
								if ( wppa( 'comneedsvote' ) ) {
									wppa_alert_text( __( "Please also give the photo a rating to get your comment published." , 'wp-photo-album-plus' ) );
								}
								...
								// Not awaiting rating, just pending
									...
									$commentblock .=
									'<span style="color:red" class="wppa-approve-'.$comment['id'].'">' .
					...
					'<table id="wppacommenttable-'.$mocc.'" style="margin:0;">' .
						...
						$txt = wppa_decode( wppa_get( 'comment', '', 'textarea' ) );
							...
							<label
								...
								>
								&nbsp;' .
								/* translators: login name or ip address */
			...
			if ( $newtag == $oldtag ) {
				...
				<tr style="border-bottom:0 none; border-top:0 none; border-left: 0 none; border-right: 0 none">
					<td class="wppa-iptc-label wppa-box-text wppa-td" style="width:50%;text-align:right">' .
						esc_js( wppa_translate( $label ) ) . '
					</td>
					<td class="wppa-iptc-value wppa-box-text wppa-td" style="width:50%;text-align:left">';
			}
			$result .= esc_js( wppa_sanitize_text( wppa_translate( $iptcline['description'], 'wp-photo-album-plus' ) ) );
			$oldtag = $newtag;
		}
	...
	if ( ! empty( $exifdata ) ) {
		...
		foreach ( $exifdata as $exifline ) {
			...
			if ( $newtag == $oldtag ) {
				...
				$result .= esc_js( wppa_translate( $label ) );
...
global $other_deps;
	...
	if ( wppa('rating_start') && is_array( $data ) ) {
		/* Translators: start date and end date */
		$text = sprintf( __('Ratings from %1$s to %2$s', 'wp-photo-album-plus'), wppa_local_date( wppa_get_option('date_format'), wppa('rating_start')), wppa_local_date( wppa_get_option('date_format'), wppa('rating_end')));
		$result .= wppa_html_tag('div', ['id' => 'bestof-period-'.wppa('mocc'), 'class' => 'bestof-period'], $text);
	}
		...
		if ( is_array( $data ) ) {
			...
			foreach ( array_keys( $data ) as $id ) {
				...
				if ( $thumb ) {
					...
					if ( $timestamp == 'yes' ) 	$totalh += $lineheight;
					...
					//			( $link['target'] ? ' target="' . $link['target'] . '"' : '' ) .
						...
						// The image
						$result .=
						wppa_html_tag( 'div', ['style' => 'height:'.$maxh.'px;width:'.$maxw.'px;'],
							wppa_html_tag( 'img', ['src' => wppa_get_photo_url( $id, true, '', $maxw, $maxh ), 'alt' => wppa_alt($id),
												   'style' => $widget ? 'height:'.$maxh.'px;width:'.$maxw.'px;' : 'max-height:'.$maxh.'px; max-width:'.$maxw.'px;margin-top:'.$tm.'px;'] )
						);
						'</div>';
						...
						// The subtitles
						$result .= "\n\t".'<div style="font-size:'.$fontsize.'px; line-height:'.$lineheight.'px; width:'.$maxw.'px">';
							/* translators: username */
							$result .= sprintf( __( 'Photo by: %s', 'wp-photo-album-plus' ), $data[$id]['user'] ).'<br>';
							if ( $timestamp 	== 'yes' ) {
								/* translators: formatted date/time */
								$result .= sprintf( __( 'Uploaded: %s', 'wp-photo-album-plus' ), wppa_local_date( wppa_get_option( 'date_format' ), wppa_get_photo_item( $id, 'timestamp' ) ) ) . '<br>';
							}
							if ( $maxratings 	== 'yes' ) {
							...
							if ( $meanrat  		== 'yes' ) {
								...
								/* translators: float nuber */
								$result .= sprintf( __( 'Rating: %4.2f.', 'wp-photo-album-plus' ), $m ) . '<br>';
							}
							if ( $totvalue 		== 'yes' ) {
								$t = $data[$id]['totvalue'];
								/* translators: integer total value */
					...
					$result .= '<div>'.sprintf( __( 'Photo %d not found.', 'wp-photo-album-plus' ), $id ).'</div>';
				}
			}
		}
		else {
					...
					<a
						...
						<div
							...
							style="border:1px solid gray;
								   ...
								   width:' . $cw . 'px"
							>' .
							wppa_translate( $date->format( 'M' ) ) . '<br>' .
							wppa_translate( $date->format( 'd' ) ) . '<br>' .
							wppa_translate( $date->format( 'D' ) ) . '<br>' .
							wppa_translate( $date->format( 'Y' ) ) . '<br>' .
							'(' . $dates[$keys[$day]] . ')' . '
						</div>
					</a>';
				}
				...
				<a
					...
					<div
						...
						style="border:1px solid gray;
							   ...
							   width:' . $cw . 'px"
						>' .
						wppa_translate( wppa_local_date( 'M', $date * $secinday ) ) . '<br>' .
						wppa_translate( wppa_local_date( 'd', $date * $secinday ) ) . '<br>' .
						wppa_translate( wppa_local_date( 'D', $date * $secinday ) ) . '<br>' .
						wppa_translate( wppa_local_date( 'Y', $date * $secinday ) ) . '<br>' .
						'(' . $dates[$keys[$day]] . ')' . '
					</div>
				</a>';
			}
...
function wppa_real_calendar_box() {
	...
	$year = wppa_get( 'calendar-year', 0, 'int' );
	$month = wppa_get( 'calendar-month', 0, 'int' );
	if ( ! $year && ! $month ) {
		$year = wppa( 'year' );
		$month = wppa( 'month' );
	}
							...
							if ( count( $thumbs ) ) {
								...
								switch( wppa_opt( 'real_calendar_linktype' ) ) {
									...
									case 'slide':
										...
										/* translators: integer count, date */
										$imgtitle = sprintf( __( 'Click for slideshow to see %1$d items of %2$s', 'wp-photo-album-plus' ), count( $thumbs ), $thisday );
										$secsinday = 24 * 60 * 60;
										if ( wppa( 'calendar' ) == 'realexifdtm' ) {
										...
										$cell_content = $the_a_tag;
											...
											if ( wppa_is_video( $id ) ) {
												...
												$cell_content .=
													wppa_html_tag( 'img', ['src' => wppa_get_thumb_url($id), 'alt' => wppa_alt($id), 'data-day' => $current_day,
																		   'id' => 'i-'.$id.'-'.$mocc, 'title' => $imgtitle, 'style' => $imgattr,
																		   'class' => 'thumb wppa-img wppa-realcalimg wppa-realcalimg-'.$mocc, 'title' => $imgtitle] );
											}
									...
									default:
										...
										/* translators: integer count, date */
										$imgtitle = sprintf( __( 'Zoom in to see %1$d items of %2$s', 'wp-photo-album-plus' ), count( $thumbs ), $thisday );
										$cell_content = '';
											...
											<a
												...
												if ( $id == $thumbs[0]['id'] ) {
													...
													if ( wppa_is_video( $thumb['id'] ) ) {
														...
														$cell_content .=
															wppa_html_tag( 'img', ['id' => 'i-'.$thumb['id'].'-'.$mocc, 'src' => wppa_get_thumb_url($id), 'title' => $imgtitle,
																				   'class' => 'thumb wppa-img wppa-realcalimg wppa-img wppa-realcalimg-'.$mocc,
																				   'data-day' => $current_day, 'style' => $imgattr, 'alt' => wppa_alt( $thumb['id'] )] );
													}
												}
...
function wppa_is_exif_date( $date ) {
	...
	if ( $t[0] > wppa_local_date( 'Y' ) ) return false;
...
function wppa_get_grid_image_html( $id ) {
	...
	if ( $link ) {
		...
		if ( $link['is_url'] ) {
			...
			if ( $is_video ) {
				$result .= wppa_get_video_html( ['id' => $id, 'controls' => wppa_switch( 'grid_video' ), 'tagid' => 'i-'.$xid.'-'.$mocc, 'title' => $title,
												 'alt' => wppa_alt($id), 'style' => $imgstyle.' cursor:pointer'] );
			}
			else {
				$result .=
				wppa_html_tag( 'img', ['id' => 'i-'.$xid.'-'.$mocc, 'src' => $imgurl, 'title' => $title, 'alt' => wppa_alt($id), 'style' => $imgstyle.' cursor:pointer'] );
			}
		...
		elseif ( $link['is_lightbox'] ) {
			...
			$result .= '<a href="'.$link['url'].'" target="'.$link['target'] . '"' .
						...
						' >';
			if ( $is_video ) {
				$result .= wppa_get_video_html( ['id' => $id, 'controls' => false, 'tagid' => 'i-'.$xid.'-'.$mocc, 'title' => wppa_zoom_in( $id ), 'style' => $imgstyle.'cursor:'.$cursor] );
			}
			else {
				$title = wppa_zoom_in( $id );
				$result .=
				wppa_html_tag( 'img', ['id' => 'i-'.$xid.'-'.$mocc, 'src' => $imgurl, 'alt' => wppa_alt($id), 'title' => $title, 'style' => $imgstyle.'cursor:'.$cursor] );
			}
	...
	// No link
		...
		if ( $is_video ) {
			$result .= wppa_get_video_html( ['id' => $id, 'controls' => wppa_switch( 'grid_video' ), 'tagid' => 'i-'.$id.'-'.$mocc, 'title' => $title, 'style' => $imgstyle] );
		}
		else {
			$result .=
			wppa_html_tag( 'img', ['id' => 'i-'.$xid.'-'.$mocc, 'src' => $imgurl, 'alt' => wppa_alt($id), 'style' => $imgstyle, 'title' => $title] );
		}
	}
