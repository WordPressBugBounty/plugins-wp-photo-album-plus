...
function wppa_make_o1_source( $id ) {
	...
	// Source available?
	if ( ! wppa_is_file( $src_path ) ) return false;
...
function wppa_make_the_photo_files( $file, $id, $ext, $do_thumb = true, $do_exif = true ) {
	...
	if ( ! $src_size ) {
		...
		else wppa_alert_text( __( 'ERROR: File is not a valid picture file.', 'wp-photo-album-plus' ) );
		return false;
	}
	...
	if ( $do_resize && wppa_can_resize( $file, max( $max_width, $max_height ) ) ) {
		...
		// Classic GD
		if ( ! wppa_can_magick() || ! wppa_is_file( $newimage ) ) {
		...
		if ( ! $done ) {
			...
			switch ( $mime ) {	// mime type
				...
				case 2:
					...
					wppa_imagejpeg( $dst, $full_thumbpath );
					break;
				case 3:
...
function wppa_is_magick( $command ) {
	...
	return wppa_is_file( rtrim( wppa_opt( 'image_magick' ), '/' ) . '/' . $command );
}
...
global $wpdb;
			...
			if ( ! $err ) {
				...
				switch( $paging ) {
					case 0: // Second pic = page #1
						/* translators: intreger pageno */
						$pagename = sprintf( __( 'Page %d', 'wp-photo-album-plus' ), $bookpage - 1 );
						break;
					case 1: // Second pic = page #2
						/* translators: intreger pageno */
						$pagename = sprintf( __( 'Page %d', 'wp-photo-album-plus' ), $bookpage );
						break;
					case 10: // Second pic = Cover - page #1
						switch ( $bookpage ) {
							...
							case 2:
								...
								break;
							default:
								/* translators: pageno, pageno */
								$pagename = sprintf( __( 'Page %1$d-%2$d', 'wp-photo-album-plus' ), 2 * $page - 2, 2 * $page - 1 );
								break;
						}
						break;
					case 11: // Second pic = Pge 2-3
						switch ( $bookpage ) {
							...
							default:
								/* translators: pageno, pageno */
								$pagename = sprintf( __( 'Page %1$d-%2$d', 'wp-photo-album-plus' ), 2 * $page, 2 * $page + 1 );
								break;
						}
						break;
					case 12: // Second pic = Pge 4-5
						/* translators: pageno, pageno */
						$pagename = sprintf( __( 'Page %1$d-%2$d', 'wp-photo-album-plus' ), 2 * $page + 2, 2 * $page + 3 );
						break;
				}
	...
	if ( wppa_is_time_up() ) {
		...
		/* translators: intreger pagecount */
	...
	$url 	= wppa_ea_url( wppa_encrypt_album( $alb ) );
	$link 	= '<a href="' . $url . '">' . $album_name . '</a>';
	wppa_update_pdf_conv_parms( $id, ['ready' => true] );
	/* translators: intreger pagecount */
	return sprintf( __( 'Conversion completed. %d pages total', 'wp-photo-album-plus' ), $page ) . ' ' .
	/* translators: link */
	sprintf( __( 'See album %s', 'wp-photo-album-plus' ), $link );
}
