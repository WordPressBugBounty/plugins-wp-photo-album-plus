...
global $wpdb;
	...
	wppaAjaxMethod = "' . wppa_opt( 'ajax_method' ) . '",
	...
	wppaLang = "' . $wppa_lang . '",
	wppaVoteForMe = "' . wppa_translate( wppa_opt( 'vote_button_text' ), 'wp-photo-album-plus' ) . '",
	wppaVotedForMe = "' . wppa_translate( wppa_opt( 'voted_button_text' ), 'wp-photo-album-plus' ) . '",
	wppaGlobalFsIconSize = "' . wppa_nav_icon_size( 'global_fs' ) . '",
	...
	wppaIconSizeSlide = "' . wppa_nav_icon_size( 'slide' ) . '",
	...
	wppaOvlIconSize = "'.wppa_nav_icon_size( 'lightbox' ).'px",
...
// global $wppa_jquery_loaded;
	...
	// No all file, do them one by one
		...
		foreach ( array_keys( $js_files ) as $idx ) {
			$js_ver = gmdate( "ymd-Gis", filemtime( plugin_dir_path( __FILE__ ) . 'js/' . $js_files[$idx] . '.js' ) );
			wp_enqueue_script( $js_files[$idx], WPPA_URL . '/js/' . $js_files[$idx] . '.js', $js_depts, $js_ver, $footer );

//			$wppa_jquery_loaded = true;
		}
		wp_localize_script( 'wppa-main', 'wppaObj', array(
			'restUrl' => rest_url(),
			'restNonce' => wp_create_nonce( 'wp_rest')
			)
		);
