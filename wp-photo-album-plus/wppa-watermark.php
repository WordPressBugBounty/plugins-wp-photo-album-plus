	...
	if ( ! $args['text'] ) {
		switch ( $args['content'] ) {
			...
			case '---name---':
				...
				break;
			case '---description---':
				$text = $id ? wp_strip_all_tags( wppa_get_photo_desc( $id ) ) : 'The Photo Description';
				break;
			case '---predef---':
...
//	if ( wppa_is_video( $id ) ) return false;					// Can not on a video
	...
	if ( ! wppa_is_file( $file ) ) return false;	// File gone
	...
	switch ( $photosize[2] ) {
		case 1: wppa_imagegif( $photoimage, $file );
			break;
		case 2: wppa_imagejpeg( $photoimage, $file );
			break;
