...
global $wppa_iptc_cache;
	...
	if ( ! empty( $iptcdata ) ) {
		foreach ( $iptcdata as $iptcline ) {
			...
			if ( $prevtag == $tag ) {			// add a next item for this tag
				$combined .= ', '.htmlspecialchars( wp_strip_all_tags( $iptcline['description'] ) );
			}
	...
	if ( $wppa_iptc_labels ) {
		foreach( $wppa_iptc_labels as $iptclabel ) {
			...
			$t = substr( $tag, 0, 2 ) . 'L' . substr( $tag, 2 );
			$tag = $t;
			$temp = str_replace( $tag, wppa_translate( $iptclabel['description'] ), $temp );
		}
	}
...
global $wppa_exif_cache;
	...
	if ( $wppa_exif_labels ) {
		foreach( $wppa_exif_labels as $exiflabel ) {
			...
			$temp = str_replace( $tag, wppa_translate( $exiflabel['description'] ), $temp );
		}
	}
...
global $wpdb;
	...
	if ( ! empty( $exifs ) ) {
		...
		foreach( $exifs as $exif ) {
			...
			$f_description 	= wp_strip_all_tags( wppa_format_exif( $exif['tag'], $exif['description'], $brand ) );
...
// Import iptc data only when configured and present
// Returns truewhen actually found data
function wppa_import_iptc( $id ) {
global $wpdb;
static $labels;

	// Do we need this?
	if ( ! wppa_switch( 'save_iptc' ) && substr( wppa_opt( 'newphoto_name_method' ), 0, 2 ) != '2#' ) return false;
	...
	$file = str_replace( '-o1.', '.', $file );
	if ( ! wppa_is_file( $file ) ) $file = wppa_get_photo_path( $id, false );
	if ( ! wppa_is_file( $file ) ) return false;
	...
	// First delete any existing iptc data for this image
	...
	foreach ( array_keys( $iptc ) as $tag ) {

		$doit = true;

		if ( is_array( $iptc[$tag] ) ) {
			...
			if ( ! in_array( $tag, $labels ) ) {
				...
				switch ( $tag ) {
					case '2#000': $desc = 'ApplicationRecordVersion:'; break;
					case '2#003': $desc = 'ObjectTypeReference:'; break;
					case '2#004': $desc = 'ObjectAttributeReference:'; break;
					case '2#005': $desc = 'ObjectName:'; break;
					case '2#007': $desc = 'EditStatus:'; break;
					case '2#008': $desc = 'EditorialUpdate:'; break;
					case '2#010': $desc = 'Urgency:'; break;
					case '2#012': $desc = 'SubjectReference:'; break;
					case '2#015': $desc = 'Category:'; break;
					case '2#020': $desc = 'SupplementalCategories:'; break;
					case '2#022': $desc = 'FixtureIdentifier:'; break;
					case '2#025': $desc = 'Keywords:'; break;
					case '2#026': $desc = 'ContentLocationCode:'; break;
					case '2#027': $desc = 'ContentLocationName:'; break;
					case '2#030': $desc = 'ReleaseDate:'; break;
					case '2#035': $desc = 'ReleaseTime:'; break;
					case '2#037': $desc = 'ExpirationDate:'; break;
					case '2#038': $desc = 'ExpirationTime:'; break;
					case '2#040': $desc = 'Spec instr:'; break;
					case '2#042': $desc = 'ActionAdvised:'; break;
					case '2#045': $desc = 'ReferenceService:'; break;
					case '2#047': $desc = 'ReferenceDate:'; break;
					case '2#050': $desc = 'ReferenceNumber:'; break;
					case '2#055': $desc = 'DateCreated:'; break;
					case '2#060': $desc = 'TimeCreated:'; break;
					case '2#062': $desc = 'DigitalCreationDate:'; break;
					case '2#063': $desc = 'DigitalCreationTime:'; break;
					case '2#065': $desc = 'OriginatingProgram:'; break;
					case '2#070': $desc = 'ProgramVersion:'; break;
					case '2#075': $desc = 'ObjectCycle:'; break;
					case '2#080': $desc = 'Photographer:'; break;
					case '2#085': $desc = 'Credit byline title:'; break;
					case '2#090': $desc = 'City:'; break;
					case '2#092': $desc = 'Sub-location:'; break;
					case '2#095': $desc = 'Procince-State:'; break;
					case '2#100': $desc = 'Country-PrimaryLocationCode:'; break;
					case '2#101': $desc = 'Country-PrimaryLocationName:'; break;
					case '2#103': $desc = 'OriginalTransmissionReference:'; break;
					case '2#105': $desc = 'Headline:'; break;
					case '2#110': $desc = 'Credit:'; break;
					case '2#115': $desc = 'Source:'; break;
					case '2#116': $desc = 'Copyright Notice:'; break;
					case '2#118': $desc = 'Contact:'; break;
					case '2#120': $desc = 'Caption-Abstract:'; break;
					case '2#121': $desc = 'LocalCaption:'; break;
					case '2#122': $desc = 'Writer-Editor:'; break;
					case '2#125': $desc = 'RasterizedCaption:'; break;
					case '2#130': $desc = 'ImageType:'; break;
					case '2#131': $desc = 'ImageOrientation:'; break;
					case '2#151': $desc = 'AudioSamplingRate:'; break;
					case '2#152': $desc = 'AudioSamplingResolution:'; break;
					case '2#153': $desc = 'AudioDuration:'; break;
					case '2#154': $desc = 'AudioOutcue:'; break;
					case '2#184': $desc = 'JobID:'; break;
					case '2#185': $desc = 'MasterDocumentID:'; break;
					case '2#186': $desc = 'ShortDocumentID:'; break;
					case '2#187': $desc = 'UniqueDocumentID:'; break;
					case '2#188': $desc = 'OwnerID:'; break;
					case '2#200': $desc = 'ObjectPreviewFileFormat:'; break;
					case '2#201': $desc = 'ObjectPreviewFileVersion:'; break;
					case '2#202': $desc = 'ObjectPreviewData:'; break;
					case '2#221': $desc = 'PhotoMechanic preferences:'; break;
					case '2#225': $desc = 'ClassifyState:'; break;
					case '2#228': $desc = 'SimilarityIndex:'; break;
					case '2#230': $desc = 'DocumentNotes:'; break;
					case '2#231': $desc = 'DocumentHistory:'; break;
					case '2#232': $desc = 'ExifCameraInfo:'; break;
					case '2#255': $desc = 'CatalogSets:'; break;
					default:
						$desc = '';
				}

				// Continue only when there is a non-empty description
				if ( $desc ) {
			...
			// Now add photo specific data item if not empty
			if ( $doit ) {
...
// Import exif data only when configured and present
// Returns true when actually found data
function wppa_import_exif( $id ) {
global $wpdb;
static $labels;
static $names;
global $wppa;
	...
	// Do we need this?
	if ( ! wppa_switch( 'save_exif' ) ) return false;
...
static $wppa_inv_filetags;
	...
	elseif ( $tagname ) {
		if ( substr( $tagname, 0, 13 ) == 'UndefinedTag:' ) {
			$tag = 'E#' . str_replace( '0x', '', substr( $tagname, 13 ) );
			return $tag;
		}
	}
	else return '';
